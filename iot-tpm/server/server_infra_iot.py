...
@app.get("/setup")
def setup():
    """Endpoint para identificar qual segredo configurar no cliente"""
    return {"user": FIXED_USER, "otp_secret": OTP_SECRET, "interval": "60s"} [cite: 1, 23]
...
def login(req: LoginRequest):
    if req.username == FIXED_USER and req.password == FIXED_PASS:
        ...
        return {"session_token": session_token, "message": "Login realizado. Envie o OTP."} [cite: 2, 24]
...
def verify(req: OTPRequest):
    ...
    if totp.verify(req.otp_code):
        return {"status": "valid", "message": "Conexão mantida."} [cite: 3, 25]
    else:
        # Derruba a sessão se o OTP falhar
        active_sessions.remove(req.token)
        raise HTTPException(status_code=401, detail="OTP inválido. Sessão encerrada.") [cite: 3, 25]
