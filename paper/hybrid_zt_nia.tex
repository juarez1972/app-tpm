...
after each \ttcmd{terraform apply}. Measured software-path costs (TOTP
...
rather than the bare code (REST \ttcmd{POST /verify} or MQTT \ttcmd{iot/verify}).
...
leaves a short replay window. Following the OTP-hardening line of
...
\end{equation}
and the client sends $\langle n, \tau, M\rangle$ instead of $\mathrm{OTP}$.
...
against the TOTP verification and network round-trip.
...
B8 & HMAC-envelope leak/replay/tamper (\ttcmd{test\_hmac\_envelope.py}) & T1040/T1550 sniff-and-replay & OTP absent from payload; reused nonce, tampered MAC, and out-of-skew $\tau$ all rejected \\
B9 & SGX enclave attestation of the Vault initializer & Untrusted-host code substitution & DCAP quote verifies; \ttcmd{MRENCLAVE}/\ttcmd{MRSIGNER} match pinned values before any share is released \\
\bottomrule
\end{tabular}
\end{table*}
